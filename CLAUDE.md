...
For a full local dev stack (bot + NodeLink + MongoDB + Piper TTS + dashboard in Docker): copy `.env.example` to `.env`, fill in `TOKEN`/`CLIENT_ID`, copy `dashboard/settings.example.json` to `dashboard/settings.json`, then `docker compose -f docker-compose.dev.yml up -d --build`. PocketTTS, the alternative announcement voice, is behind `--profile pocket` because it pulls in torch. The entrypoint creates `settings.json` from `settings.docker.json` (service-name endpoints) on first boot; the project dir is bind-mounted, so code changes only need a `docker compose restart vocard`.

The stack runs [NodeLink](https://github.com/PerformanC/NodeLink) rather than Lavalink: it speaks the same v4 REST/WS API but has built-in sources (no youtube-plugin/LavaSrc) and an audio mixer (`POST /v4/sessions/{s}/players/{g}/mix`, body `{"track": {"encoded": ...}, "volume": 0.0-1.0}`) that the TTS announcer uses to play clips *over* the current track. `docker-compose.lavalink.yml` is the Lavalink fallback; the announcer detects a missing mixer and degrades to fade mode.
